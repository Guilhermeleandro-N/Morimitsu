...
import { listarFrequenciasAluno, editarFrequencia } from "../../services/frequenciaService";
...
const PerfilProfessor = () => {
  ...
  async function togglePresenca(item) {
    const novoStatus = item.status_presenca === "PRESENTE" ? "AUSENTE" : "PRESENTE";
    setHistorico((prev) =>
      prev.map((freq) =>
    ...
    try {
      ...
      setHistorico((prev) =>
        prev.map((freq) =>
          freq.id === item.id ? { ...freq, status_presenca: item.status_presenca } : freq
        )
      );
    }
  }
      ...
      <div className="container">
        <header className="page-header">
          ...
          <RoleGuard allowedRoutes={["admin"]}>
            <button
              className="btn-edit"
              onClick={() => navigate("/editarProfessor", { state: professorData })}
            >
              Editar Professor
            </button>
          </RoleGuard>
        ...
        <main className="profile-grid">
          <aside className="sidebar-card">
            ...
            <RoleGuard allowedRoutes={["professor", "aluno"]}>
              <span className={`status-badge ${String(dados.status || "enabled").toLowerCase()}`}>
                {statusLabel(dados.status)}
              </span>
            </RoleGuard>
            <div className="personal-details">
              ...
              <p><strong>Faixa:</strong> {dados.faixa || "--"}</p>
              <p><strong>Grau:</strong> {dados.grau ?? "--"}</p>
            </div>
          </aside>
                ...
                {historico.length > 0 ? (
                  historico.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>{new Date(item.horario_inicio).toLocaleTimeString("pt-BR")}</td>
                      <td>{new Date(item.horario_fim).toLocaleTimeString("pt-BR")}</td>
